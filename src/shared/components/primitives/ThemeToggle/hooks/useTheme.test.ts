/**
 * Pruebas de `useTheme` — `localStorage`, clase `dark`, meta `theme-color` y `prefers-color-scheme` sin valor guardado.
 *
 * @fileoverview `renderHook` con `matchMedia`, `localStorage` y DOM (`documentElement`, `<meta>`) mockeados o espiados.
 * @remarks Incluye sub-suite que distingue listener de sistema según haya o no tema persistido en storage.
 */
